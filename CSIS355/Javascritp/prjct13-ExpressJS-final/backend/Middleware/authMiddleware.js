...
const protect = asyncHandler(async (req, res, next) => { // Middleware function 

// sits between the request and the route handler, blocks unauthorized access
...
let token // Declares token variable in outer scope so both if-blocks below can access it
  ...
  if (
    ...
    req.headers.authorization.startsWith('Bearer')
     ...
     // Checks it follows the Bearer token format — e.g. "Bearer eyJhbG..."
